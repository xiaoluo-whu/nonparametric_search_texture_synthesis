...
{//合成N幅纹理图像
         int N=StrToInt(this->Edit3->Text);
               for(int q=0;q<N;q++)
           {
               ...
               jpg->LoadFromFile("d://我的文档/桌面/纹理子图采集/检材/"+IntToStr(0)+"_"+IntToStr(0)+IntToStr((q+1)/10)+IntToStr((q+1)%10)+".jpg");
               bmp->Assign(jpg);

               AnsiString mmid = ExtractFileName("d://我的文档/桌面/纹理子图采集/检材/"+IntToStr(0)+"_"+IntToStr(0)+IntToStr((q+1)/10)+IntToStr((q+1)%10)+".jpg").SubString(1,5) ;
          ...
          for (int js = 0 ; js < 1 ; js ++)
            {
               ...
               for (int y = 0 ; y < h ; y ++)
                ...
                ByteToBmp(repic  , temp);
                 ...
                 pic1->Assign(repic);
                 AnsiString dir="e://texture/"+IntToStr(0)+"-"+IntToStr(q)+".bmp";
                 pic1->SaveToFile(dir);
   ...
   this->ListBox2->Clear();
    for(int q=0;q<N;q++)
     ...
     for (int q=0;q<N;q++)
       {for(int i=0;i<160;i++)
         {
           this->ListBox2->Items->Add(FloatToStr(glcmfea[q][i]));
         }
       }
   delete bmp;
